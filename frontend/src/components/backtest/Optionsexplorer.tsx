...
// Der Optionsbacktest ueber alle Aktien (ADR 0058) -- unveraendert aus der
// ersten Fassung der Backtest-Seite, nur als Bereich des Explorers.
      ...
      <tbody>
        {zeilen.map((zeile) => (
          ...
          <tr key={zeile.stock_id} className={zeile.managed === null ? 'duenn' : undefined}>
            ...
            <td className="zahl">{formatProzent(zeile.held?.mean_return_on_capital ?? null, 2)}</td>
            <td className="zahl">
...
function Kombinationstabelle({ zeilen }: { zeilen: readonly Kombinationsergebnis[] }): ReactNode {
  if (zeilen.length === 0) {
    return (
...
export function Optionsexplorer(): ReactNode {
  ...
  return (
    <>
          ...
          >
            {messungen.map((messung) => (
              <option key={messung.measurement_id} value={messung.measurement_id}>
                ...
                {messung.assumptions.volatilitaetsaufschlag ?? '?'} · {messung.stocks} Aktien
              </option>
            ))}
          </select>
        </p>
      ...
      {/* Ohne diese Zeile sähe der Wechsel auf eine andere Messung aus wie
          ...
          Messung vor". */}
      {gewaehlt !== null && detail === null && fehler === null && <p>Messung wird geladen …</p>}
      {detail !== null && (
        <>
          ...
          >
            {reiter === 'aktien' ? (
              <Aktientabelle zeilen={detail.stocks} messungId={detail.measurement.measurement_id} />
            ) : (

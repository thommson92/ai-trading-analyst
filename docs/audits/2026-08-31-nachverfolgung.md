...
| A2-F008 | Contract-Tests nur für EDGAR eingefroren | **teilweise** | A2-M7: Werkzeug und Anleitung stehen, die echten Antworten fehlen noch |
...
| A2-M7 | Contract-Antworten einfrieren (Finnhub ×2, IBKR-Optionskette) | **teilweise** | Werkzeug und Anleitung stehen: Mitschnitt der TWS-Antworten (`infrastructure/ibkr/chain_recorder.py`, Schalter `cli options --record`) und Doc 14, Zwischenschritt „Contract-Antworten einfrieren" mit allen drei Kommandos. Offen: die drei echten Dateien und die Tests darauf — sie brauchen den Serverkontakt (Finnhub-Schlüssel, laufende TWS bei offenem Markt) |
